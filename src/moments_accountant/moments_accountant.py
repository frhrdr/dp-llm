...
def compute_b(sigma, q, lmbd, verbose=False):
    ...
    def b_fn(z):
        return np.power(mu0(z) / mu(z), lmbd) - np.power(mu(-z) / mu0(z), lmbd)
    ...
    def b_lambda_fn(z):
        return mu0(z) * np.power(cropped_ratio(mu0(z), mu(z)), lmbd)

    def b_lambda_int1_fn(z):
        return mu0(z) * np.power(cropped_ratio(mu0(z), mu(z)), lmbd)

    def b_lambda_int2_fn(z):
        return mu0(z) * np.power(cropped_ratio(mu(z), mu0(z)), lmbd)
...
def compute_a_mp(sigma, q, lmbd, verbose=False):
    ...
    def a_lambda_fn(z):
        return mu(z) * (mu(z) / mu0(z)) ** lmbd_int
...
def compute_b_mp(sigma, q, lmbd, verbose=False):
    ...
    def b_lambda_fn(z):
        return mu0(z) * (mu0(z) / mu(z)) ** lmbd_int

    def b_lambda_int1_fn(z):
        return mu0(z) * (mu0(z) / mu(z)) ** lmbd_int

    def b_lambda_int2_fn(z):
        return mu0(z) * (mu(z) / mu0(z)) ** lmbd_int
